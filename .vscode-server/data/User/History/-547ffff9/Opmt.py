...
def get_courses():
    ...
    courses_html = "<h1>Courses</h1><ul>"
    for course in courses:
        courses_html += f"<li>{course['name']} (Instructor: {course['instructor']}, Location: {course['location']}, Time: {course['time']})</li>"
    courses_html += "</ul>"
...
@app.route('/get_course_details/<int:course_id>', methods=['GET'])
def get_course_details(course_id):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT instructor, location, time FROM courses WHERE id = %s", (course_id,))
    ...
    if course:
        ...
        return jsonify({'instructor': '', 'location': '', 'time': ''}), 404
